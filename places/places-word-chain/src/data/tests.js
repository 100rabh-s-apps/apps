// // Load the compressed trie JSON (e.g., fetched or imported)
...
function searchPrefix(trie, prefix) {
  const { node, seed } = locateNodeAtPrefix(trie, prefix);
  if (!node) return [];
  const results = [];
...
// Load the compressed trie JSON file
function loadTrie(filepath) {
...
(async () => {
  const trie = await loadTrie("compressed_trie_output.json");

  console.log("Searching for places starting with 'A':");
  const matches = searchPrefix(trie, "california");
  console.log(matches);
})();
